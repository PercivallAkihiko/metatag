...
function generatePoll(results) {
  ...
  if (results.length < 5) {
    pollContainer.style.gridTemplateRows =
      "30px repeat(" + results.length + ", 60px) auto";
  } else {
    pollContainer.style.gridTemplateRows =
      "30px repeat(" + results.length + ", auto)";
  }
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    resultVote.style.gridTemplateColumns =
      fullFraction + "fr " + emptyFraction + "fr";
...
function suggestTag(event) {
  ...
  const matchingTags = validTags.filter((tag) =>
...
function addTagFromCookie(videoId, disabled = false, first = false) {
  ...
  if (cookieValue === null && first) {
    suggestionsContainer.innerHTML =
      "<grey>Seed: </grey> <cursive>submit to generate.</cursive>";
    return;
  }
  if (cookieValue === null) {
    suggestionsContainer.innerHTML =
      "<grey>Seed: </grey> <cursive>missing cookies.</cursive>";
...
function generateChart(days, ethprice, tokens) {
  var fill = Array.from(
    { length: Math.max(30 - tokens.length, 0) },
    () => 0
  ).concat(tokens);
